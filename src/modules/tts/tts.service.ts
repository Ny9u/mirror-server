...
// 定义 TTS Client 类型
type TtsClient = InstanceType<typeof Tts.tts.v20190823.Client>;

// 扩展 TTS 响应类型，包含可能的错误信息
...
export class TTSService {
  ...
  async textToSpeech(
    ...
    try {
      ...
      const response = (await this.ttsClient.TextToVoice(
        params
      )) as TTSApiResponse;
      ...
      if (!response.Audio) {
        throw new Error("TTS API返回的音频数据为空");
      }
      ...
      this.logger.error(`文本转语音失败: ${errorMessage}`);
      throw new Error(`文本转语音失败: ${errorMessage}`);
    }
  }
